...
# Authorization header for service bootstrap user
...
class AverageHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        requestHeaders = {
            'Authorization': getAuthHeader(extractTenant(credentials))
        }
...
'''
Handler to return the content of injected headers
'''
...
'''
Query the API for subscribed tenants and service users
'''
def getSubscriptions():
    logging.debug('Poll for subscriptions')
    response = requests.get(C8Y_BASEURL + '/application/currentApplication/subscriptions', headers={'Authorization': BOOTSTRAP_AUTHENTHICATION_HEADER})
...
'''
Poll the subscribed tenants and service users in a loop
'''
...
'''
Find correct autentication header of service user for given tenant
'''
def getAuthHeader(tenant):
    return APPLICATION_SUBSCRIPTIONS[tenant]['header']

'''
Extract the tenant of a base64 authentication
'''
def extractTenant(credentials):
    credentials = base64.b64decode(credentials.split(' ', 1)[1]).decode('utf-8')
...
# Schedule subscription polling
LOOP = asyncio.get_event_loop()
LOOP.run_in_executor(None, pollSubscriptions)
